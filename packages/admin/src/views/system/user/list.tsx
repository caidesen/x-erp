...
export function Component() {
	const { mutateAsync: createUser } = useMutation({
		mutationFn: api.system.user.create,
		onSuccess: () => {
			return actionRef.current?.reload(true);
		},
	});
	const { mutateAsync: updateUser } = useMutation({
		mutationFn: api.system.user.update,
		onSuccess() {
	...
	const { mutateAsync: deleteUsers } = useMutation({
		mutationFn: api.system.user.batchRemove,
		onSuccess() {
	...
	const { data: roleList } = useQuery({
		queryKey: [api.system.role.all.cacheKey],
		queryFn: () => api.system.role.all(),
	});
	return (
		<PageContainer>
			<ProTable
				...
				request={async () => {
					const data = await api.system.user.all();
					return {
						data: data.map((it) => ({
							...it,
